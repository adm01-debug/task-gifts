...
import { Plus, Download, Upload, RefreshCw, CheckSquare, Play, Pause } from 'lucide-react';
...
export const TarefasToolbar = memo(function TarefasToolbar({ onSearch, onFiltersChange, onRefresh, onNewClick, selectedCount, onClearSelection, onBulkConcluir, onBulkIniciar, currentFilters, data = [] }: TarefasToolbarProps) {
  const [filterValues, setFilterValues] = useState<FilterValue[]>([]);

  const handleImport = async (tarefas: unknown[]) => {
    const { error } = await supabase.from('tarefas').insert(tarefas);
    if (error) throw error;
    toast.success(`${tarefas.length} tarefas importadas!`);
    onRefresh();
  };
  ...
  const handleExport = () => {
    ...
    exportToExcel(data as Record<string, unknown>[], [
      ...
      { key: 'responsavel_nome' as const, label: 'Responsável' },
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex flex-wrap gap-2 items-center">
          <SearchInput onSearch={onSearch} placeholder="Buscar tarefa..." className="w-64" />
          <AdvancedFilters filters={taskGiftsFilterConfigs.tarefas as FilterConfig[]} values={filterValues} onChange={(v) => { setFilterValues(v); onFiltersChange(v); }} />
          <SavedFiltersDropdown entityType="tarefas" currentFilters={currentFilters} onApplyFilter={(f) => { const values = Object.entries(f).map(([k,v]) => ({ key: k, operator: 'eq' as const, value: v })); setFilterValues(values); onFiltersChange(values); }} />
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={onRefresh}><RefreshCw className="h-4 w-4" /></Button>
          <DataImporter schema={tarefaSchema} columns={taskGiftsImportTemplates.tarefas} onImport={handleImport} templateName="tarefas" title="Importar Tarefas" trigger={<Button variant="outline" size="sm"><Upload className="h-4 w-4" /></Button>} onSuccess={onRefresh} />
