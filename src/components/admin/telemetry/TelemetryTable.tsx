...
import { type TelemetryRow, formatDuration, formatTime } from "@/pages/AdminTelemetria";
...
export function TelemetryTable({ rows, isLoading, getSeverityBadge }: TelemetryTableProps) {
  return (
    <Card>
...
interface TopOffendersProps {
  offenders: [string, { count: number; totalMs: number; maxMs: number }][];
}
    ...
    <Card>
      <div className="p-4 pb-3">
        <h3 className="text-base font-semibold flex items-center gap-2">
          <Database className="h-4 w-4" />Tabelas Mais Problemáticas
        </h3>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {offenders.map(([name, stats]) => (
            <div key={name} className="p-3 rounded-lg border border-border/50 bg-muted/30">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">média: {formatDuration(Math.round(stats.totalMs / stats.count))}</p>
            </div>
          ))}
        </div>
      </CardContent>
